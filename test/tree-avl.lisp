...
(define-test avl-tree/make-tree
  (let ((tree (tree:make-tree 'tree:avl-tree)))
    (of-type tree:avl-tree tree)
    (false (tree::node-p (tree::root tree)))
    (is eq #'identity (tree::key tree))
    (is eq #'< (tree::test tree))))

(define-test avl-tree/valid-p
  (dotimes (i 10)
    (let ((tree (tree:make-tree 'tree:avl-tree)))
      (dotimes (i 1000)
...
(define-test avl-tree/find
  (let ((tree (tree:make-tree 'tree:avl-tree)))
    ...
    (setf tree (tree:make-tree 'tree:avl-tree :key #'cdr))
